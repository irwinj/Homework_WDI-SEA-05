...
describe Item do
  ...
  describe "Initialization" do
    ...
    it "is assigned a name" do
      expect(@item2.name).to eq("name")
    end
    it "is assigned a price" do
      expect(@item2.price).to eq(1.99)
    end
  ...
  describe "Accessors" do
    ...
    it "should be able to get and set weight but default to 0" do
      expect(@item.weight).to eq(0)
      @item.weight = 2.2
      expect(@item.weight).to eq(2.2)
    end  
  end
  ...
  describe "Methods" do
    ...
    it "should be able to return an item and update quantity" do
      result = @item.return 1
      expect(result).to eq(true)
      expect(@item.quantity).to eq(3)
    end
    it "should return the correct shipping price based on weight" do
      @item.weight = 12
      expect(@item.ship_price).to eq(14.399999999999999)
    end
  end
